...
class OpenClashUrlResolverTest {
    ...
    fun `uses router host and cn port when no forward config`() {
        ...
        assertEquals(
            "http://192.168.1.1:9090/ui/zashboard/#/setup?hostname=192.168.1.1&port=9090&secret=secret123",
    ...
    fun `uses forwarded https endpoint when configured`() {
        ...
        assertEquals(
            "https://panel.example.com:9443/ui/zashboard/#/setup?hostname=panel.example.com&port=9443&secret=forward-secret",
            resolved.zashboardUrl,
        )
